...
annular approximation to the Mandel--Agol forward model, never seeing a radius label. It attains
$\mathrm{MAE}=0.0100$ in $R_p/R_*$, within $24\%$ of the best fully supervised model,
and occupies a distinct region of the Pareto plane under matched simulation. Applied
zero-shot to twelve confirmed Kepler planets it recovers radii with full-sample
$\mathrm{MAE}=0.023$, a feasibility result rather than a reliability estimate. We
...
\hypersetup{
  pdfauthor={Bhavya Gondra, Akhil Muvvala, Milan Petrov, Harish Senthil, Ayan Sharma, Manav Sharma, Pranavan Sivaraman, Adhrit Somayajula, Srikanth Samy, Ishaan Gupta},
  pdfkeywords={exoplanets, interpretable machine learning, Kolmogorov-Arnold networks, physics-informed learning, transit photometry, model comparison},
  pdflang={en-US}}
...
axis $a/R_*$ and impact parameter $b$, define phase as
$\phi=(t-t_0)/P$, with the simulation period normalised to $P=1$. The projected
star--planet separation is
\begin{equation}
...
For $500$ parameter draws, its maximum/mean absolute discrepancies and RMSE relative to
\texttt{batman} are $5.53\times10^{-6}$/$1.53\times10^{-7}$/$5.50\times10^{-7}$.
...
\Description{Four synthetic transit profiles illustrate how radius ratio, impact
parameter, orbital scale, and epoch alter depth, duration, shape, and timing.}
\label{fig:shapes}
\end{figure}
...
duration. This is a simulation-specific white-noise proxy, not a claim about survey
detection completeness under correlated noise or systematics. The training set holds
$8{,}000$ segments ($4{,}006$ positive), the disjoint
...
This describes the single-layer $[201,1]$ KAN ($5{,}632$ parameters; cubic splines,
grid $5$). The physics-KAN is a two-layer $[201,8,4]$ model ($28{,}796$ parameters), not
a GAM; it is assessed by surrogate fidelity. Only the single-layer KAN's $\psi_j$ are
directly plotted without post-hoc approximation~\cite{ribeiro2016lime}.

The MLP/CNN/LSTM have $34{,}177$/$3{,}233$/$9{,}025$ parameters and train for
$30$/$30$/$18$ epochs at $10^{-3}$; the single-layer and physics KANs train for $40$ at
$5\times10^{-3}$ and $80$ at $3\times10^{-3}$ (batch $256$). Forest/XGBoost use $200$
trees (XGBoost depth $4$), and ridge $\alpha=1$. Settings were fixed a priori without
tuning or compute matching. The physics-KAN also evaluates $128$ annuli per batch;
wall-clock times were not logged.
...
\Description{Two ranked horizontal bar charts compare model families on detection AUC
and radius-ratio mean absolute error.}
...
As a limited optimisation-variance check, we retrained the leading MLP with five seeds
on the same split. Test AUC is $0.9751\pm0.0009$ (sample SD), with range
$[0.9736,0.9757]$; its $0.029$ gap from the linear model is therefore much larger than
the observed MLP seed variation. Other families and the physics-KAN were not replicated,
so their pairwise differences remain single-seed results.
...
in \eqref{eq:physloss}. Radius strongly controls depth, but it is not uniquely
identifiable from depth: $\rho$, $b$, $a/R_*$, limb darkening, and $t_0$ jointly shape
depth, duration, and ingress. The bounded joint fit exploits all $201$ samples, while
remaining subject to these degeneracies---the same inverse-problem logic that underlies
physics-informed neural networks~\cite{raissi2019pinn}.

A finite-difference Jacobian check quantifies this coupling: range-scaled median RMS
flux sensitivities over $200$ curves are $0.00618$ ($\rho$), $0.00261$ ($b$), $0.00303$
($a/R_*$), and $0.00819$ ($t_0$). With geometry known, fitting $\rho$ while assuming
$(u_1,u_2)=(0.3,0.2)$ gives MAE $0.00194$ for true $(0.1,0.1)$ and $0.00240$ for true
$(0.5,0.3)$ over $120$ curves (maxima $0.00855$/$0.01095$). Unknown geometry can add
error, so these are diagnostics, not robustness guarantees.
...
frontier is long: full readability costs $85\%$ higher error than the best black box,
while the physics-informed model occupies an intermediate position.}
\Description{Two scatterplots place each model on performance-versus-additive-fidelity
axes and connect the non-dominated models with dashed Pareto frontiers.}
\label{fig:pareto}
\end{figure}
...
\Description{One panel overlays a reconstructed transit on an observed Kepler fold; the
other compares predicted and published radius ratios for twelve planets.}
\label{fig:kepler}
\end{figure}
...
\item Zero-shot transfer to twelve real Kepler folds is feasible
      ($\mathrm{MAE}=0.023$) but is not evidence of population-level reliability; its
      dominant error is underestimation of deep transits under long-cadence smearing.
...
decoder is the very process that generated the synthetic data. The limb-darkening
sensitivity check and Kepler results show how accuracy degrades when that match is
broken, but do not span realistic mismatch. Additive-surrogate
...
Only the MLP detection result is repeated across five seeds; all other training results
carry unquantified optimisation variance, so close pairwise differences should not be
treated as meaningful.

\subsection{Future work}
The next experiment should test the current model under correlated noise, limb-darkening
mismatch, and long-cadence integration; adding the latter to Eq.~\eqref{eq:flux} should
address the deep-transit bias in \cref{fig:kepler}. TESS~\cite{ricker2015tess} and
single-transit data~\cite{foremanmackey2016} would then test broader transfer.

\section{Conclusion}
On simulated transits, full transparency costs $0.029$ detection AUC and $85\%$ more
radius error. A physics-informed alternative reaches near-supervised accuracy without
parameter labels under matched simulation. Its twelve-planet transfer is a feasibility
check that exposes instrument mismatch, not a validated catalogue-level estimator.

\section*{Reproducibility}
Code, notebooks, and fixed inputs are in the
\href{https://github.com/guptaishaan/research-projects/tree/icaise-2026-format/exoplanet-model-comparison}{public repository}.
The figure script regenerates the figures, tables, and residual screen; the notebook
validates Eq.~\eqref{eq:flux} against
\texttt{batman}~\cite{kreidberg2015batman} and fixes all data-generation seeds. Committed
outputs used NumPy 2.5.1, pandas 2.3.3, Matplotlib 3.11.0, scikit-learn 1.9.0,
XGBoost 2.1.4, PyTorch 2.6.0+cu124, \texttt{batman} 2.5.3, and pykan 0.2.8.
