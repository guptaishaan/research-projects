...
\hypersetup{
  pdfauthor={Isha Gupta, Ishaan Mohandas, Ashwath Raghu, Jake Rajbhandari, Anya Samuel, Riva Sheth, Mridhula Suresh, Srikanth Samy, Ishaan Gupta},
  pdfkeywords={brain-computer interfaces, EEG, explainability, perturbation tests, simulation, validation},
  pdflang={en-US}}
...
\Description{Three panels show the electrode montage, power spectra with and without the
planted ten-hertz signal, and a scalp map localising the constructed class contrast.}
\label{fig:sim}
\end{figure}
...
\Description{Two heatmaps compare decoder fire rates when a perturbation removes a real
target and when it removes an absent target.}
\label{fig:toy}
\end{figure}
...
\Description{Paired heatmaps show sensitivity and false-alarm rates for four decoder
families across band and spatial perturbations.}
\label{fig:oc}
\end{figure}
...
\Description{A categorical heatmap assigns trust or suspicion to each real-decoder
perturbation result, alongside a bar chart of held-out balanced accuracy.}
\label{fig:audit}
\end{figure}
...
\Description{A two-column heatmap grades band-power and CSP perturbations against known
eyes-open versus eyes-closed physiology and prints the corresponding accuracy drops.}
\label{fig:eoec}
\end{figure}
