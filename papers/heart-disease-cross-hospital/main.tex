...
\begin{abstract}
A clinical risk model can perform well internally yet transfer poorly to a hospital
with a different case mix, measurement practice, or missing-data pattern. We study this
failure mode on the
...
Cleveland discrimination reaches $\mathrm{AUC}=0.910$ for the random forest. Transfer
performance is heterogeneous: directed pairs range from $\mathrm{AUC}=0.571$ to
$0.888$. The random forest has macro-average external AUC $0.781$ and internal AUC
$0.756$; this reflects the populations being averaged and does not imply improvement
under transfer. Logistic regression has a $0.028$ internal-to-external decrease. The
MLP changes little but performs worse in absolute terms. We argue that the scalar
...
\hypersetup{
  pdfauthor={Anoushka Aisola, Vihaan Bhoopathy, Rayan Hossain, Spoorthi Idumudi, Nethra Kollipara, Rania Malik, Nishant Nayagam, Yukti Srinivas, Srikanth Samy, Ishaan Gupta},
  pdfkeywords={clinical machine learning, dataset shift, external validation, heart disease, ROC analysis, reproducibility, transportability},
  pdflang={en-US}}
...
institution with a shared instrument but different recording practices. Its four-site
structure is useful for demonstrating validation design, but its small, historical
cohorts cannot support claims about contemporary clinical performance.
...
The internal and transport studies use the predictors listed in \cref{tab:predictors}.

\begin{table}[t]
\centering
\caption{Predictors and feature-set membership. $\mathcal{F}_{13}$ is used for internal
Cleveland validation; $\mathcal{F}_{9}$ contains variables with coverage at all sites.}
\label{tab:predictors}
\scriptsize
\begin{tabular}{@{}lp{0.55\linewidth}cc@{}}
\toprule
Code & Predictor & $\mathcal{F}_{13}$ & $\mathcal{F}_{9}$\\
\midrule
\texttt{age} & Age (years) & yes & yes\\
\texttt{sex} & Sex code & yes & yes\\
\texttt{cp} & Chest-pain type & yes & yes\\
\texttt{trestbps} & Resting blood pressure & yes & yes\\
\texttt{chol} & Serum cholesterol & yes & --\\
\texttt{fbs} & Fasting blood sugar $>120$ mg/dL & yes & yes\\
\texttt{restecg} & Resting electrocardiogram category & yes & yes\\
\texttt{thalach} & Maximum heart rate achieved & yes & yes\\
\texttt{exang} & Exercise-induced angina & yes & yes\\
\texttt{oldpeak} & Exercise-induced ST depression & yes & yes\\
\texttt{slope} & Slope of peak exercise ST segment & yes & --\\
\texttt{ca} & Major vessels observed by fluoroscopy & yes & --\\
\texttt{thal} & Thalassemia/test-result category & yes & --\\
\bottomrule
\end{tabular}
\end{table}
...
\Description{A grouped bar chart compares accuracy, ROC-AUC, and F1 for logistic
regression, random forest, and a multilayer perceptron on Cleveland.}
\label{fig:internal}
\end{figure}

Evaluated alone, \cref{tab:internal} suggests strong discrimination within Cleveland
($\mathrm{AUC}\approx0.91$). It provides no evidence of performance outside that cohort.
...
\Description{Three four-by-four heatmaps show directed source-to-target AUC for logistic
regression, random forest, and the multilayer perceptron.}
\label{fig:matrix}
\end{figure}
...
Switzerland performs poorly as a \emph{source} for logistic regression and the MLP: with
...
model dominates. Selecting a model using only one held-out target would produce different
rankings for different target cohorts. This motivates reporting the complete matrix.
...
\Description{Grouped bars compare the three classifiers across all twelve directed
hospital-transfer pairs.}
\label{fig:directed}
\end{figure}
...
over different populations: the internal average is lowered by Switzerland's
...
\Description{Two grouped bar charts compare macro internal and external AUC and display
their signed difference for each classifier.}
\label{fig:gap}
\end{figure}

The MLP illustrates a complementary limitation. Its gap is the smallest in magnitude
...
model with uniformly lower performance can have a small gap by construction. Gap
...
\section{Conclusion}
External validity cannot be established by one held-out split. On the four UCI
heart-disease cohorts, a model with
...
report absolute target performance, and model source-cohort recording quality as an
explicit experimental variable.

\section*{Data and Code Availability}
The cohorts are the four public UCI Heart Disease files: \texttt{processed.cleveland.data},
\texttt{processed.hungarian.data}, \texttt{processed.switzerland.data}, and
\texttt{processed.va.data}. The analysis notebook,
