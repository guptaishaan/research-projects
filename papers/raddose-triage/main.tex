...
\title{When the Target Is Wrong}
\subtitle{Reframing Plant Radiation Dosimetry as Cross-Study Exposure Triage}
...
\keywords{Arabidopsis, domain generalization, gene expression, leave-one-study-out
validation, radiation biology, study design, triage}

\maketitle

\hypersetup{
  pdfauthor={Sanvi Desai, Sushant Donadi, Gayatri Goli, Soham Gupta, Aswath Madhan, Jackson Szekeres, Srikanth Samy, Ishaan Gupta},
  pdfkeywords={Arabidopsis, domain generalization, gene expression, leave-one-study-out validation, radiation biology, study design, triage},
  pdflang={en-US}}
...
\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig1_design_audit.pdf}
...
exposure with batch.}
\Description{Two stacked bar charts show sample counts by radiation dose and exposed
versus control composition for each of six studies.}
\label{fig:audit}
\end{figure}
...
require a reference-based normalization that can transform a single incoming sample.
Concretely, a reference panel must be frozen using training studies; library-size or
variance-stabilising transforms, per-gene centres and scales, and quality-control limits
must then be learned from that panel and applied unchanged to each incoming sample.
External spike-ins or validated housekeeping genes could anchor the transform, while
reference samples processed with every laboratory batch would monitor drift. No cohort
statistics from the incoming study may enter prediction. Developing and prospectively
validating that single-sample transform is the principal gap between this benchmark and
a usable assay.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.86\linewidth]{fig2_model_ladder.pdf}
...
\Description{Horizontal bars compare six models on accuracy, balanced accuracy,
...
\Description{A confusion-matrix heatmap summarizes pooled predictions, and grouped bars
compare balanced accuracy and ROC-AUC across six held-out studies.}
\label{fig:errors}
\end{figure}
...
these points are descriptive only. Dose is also confounded with study, radiation quality,
and tissue, so neither monotonicity nor a dose--response relation can be inferred. The
only supported reading is that the classifier is not purely a $100$~Gy detector.

\begin{figure}[t]
\centering
\includegraphics[width=\linewidth]{fig4_dose_and_roc.pdf}
...
\Description{A dose-versus-score line plot with uncertainty bars is paired with six
study-specific receiver operating characteristic curves.}
\label{fig:dose}
\end{figure}
...
study. Across all $4{,}002$ genes, $186$ enter at least one top-100 panel: $72$ are
selected in one fold, $19$ in two, $8$ in three, $16$ in four, $24$ in five, and $47$
in all six (median frequency $3$ among selected genes). \cref{fig:genes} shows the most
stable subset. Three canonical DNA-damage-response genes---AT4G02390, AT4G22960, and
AT5G66130---are selected in all six outer folds, alongside cell-cycle, repair, and
heat-shock annotated genes.

\begin{figure}[t]
\centering
\includegraphics[width=0.78\linewidth]{fig5_stable_genes.pdf}
...
\Description{A horizontal bar chart ranks genes by the number of outer folds in which
each gene appears in the selected panel.}
\label{fig:genes}
\end{figure}

Ionising radiation can generate reactive oxygen species and DNA lesions, activating
damage sensing, repair, cell-cycle arrest, and general stress programmes. The observed
cell-cycle, repair, and heat-shock annotations therefore agree with known plant
radiation biology~\cite{culligan2006,missirian2014} and make the classifier biologically
plausible.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.52\linewidth]{fig6_dose_regression.pdf}
...
\Description{A scatterplot compares predicted with true radiation dose and shows most
predictions compressed toward the middle of the zero-to-one-hundred-gray range.}
...
Deployment would also cross several untested boundaries: sequencing platform and
laboratory, genotype and tissue, radiation quality and time since exposure, and the
prevalence-dependent calibration of the fixed $0.5$ threshold. A deployable screen
therefore requires a frozen single-sample normalization, laboratory quality controls,
prospective calibration, an abstention rule for out-of-distribution samples, and
confirmation of positive calls by an independent assay. The present classifier is a
cross-study research benchmark, not a field-ready diagnostic.
...
$0.5$ threshold have been frozen. A Fisher-$z$ calculation requires about $29$
independent samples for $80\%$ power at two-sided $\alpha=0.05$ to detect a correlation
of $0.5$. We therefore recommend at least $30$ independent biological replicates across
at least five prespecified doses (at least six per dose) as a minimum confirmatory curve,
preferably repeated in more than one study. This is a planning lower bound, not enough
by itself to establish cross-study generalisation. The experiment would also test the
specificity claim that the present data cannot address.
...
The repository contains the harmonized matrix, sample manifest, raw-study examples,
annotation tables, and environment specification in the
\href{https://github.com/guptaishaan/research-projects/tree/icaise-2026-format/raddose-phytodosimetry}{public repository}.
The figure script accompanying this manuscript
...
CPU, using scikit-learn~\cite{pedregosa2011}. The requirements specify Python 3.11 and
minimum versions NumPy 1.24, pandas 2.0, Matplotlib 3.7, scikit-learn 1.3, SciPy 1.10,
and PyArrow 12. Seeds, fold definitions, feature counts, hyperparameters, target
definition, and threshold are fixed in code.
